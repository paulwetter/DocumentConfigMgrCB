<#
Author: David O'Brien, Dilignet, david.obrien@dilignet.com
Version: 1.0
Version History: 1.0 released on 24/10/2014
...
.\Update-AlwaysON_v3.ps1 -SQLNodes @('KWTSQL352','KWTSQL353') -AlwaysOnInstance MSSQLSERVER -AvailabilityGroupName 'AG_HYP_01' -CollectionID KP10001B -SMSProvider kwpcmp001.core.kmtltd.net.au -Verbose

.\Update-AlwaysON_v3.ps1 -SQLNodes @('KWTSQL352','KWTSQL353') -AvailabilityGroupName 'AG_HYP_01' -CollectionID KP10001B -SMSProvider kwpcmp001.core.kmtltd.net.au -Verbose

When Email is needed at the end of the process, use this command line and add the variables in lines 40-43:
.\Update-AlwaysON_v3.ps1 -SQLNodes @('KWTSQL352','KWTSQL353') -AvailabilityGroupName 'AG_HYP_01' -CollectionID KP10001B -SMSProvider kwpcmp001.core.kmtltd.net.au -SendEmail -Verbose 
...
<#
$SQLNodes = @('KWTSQL352','KWTSQL353')
#$AlwaysOnInstance = 'AG_HYP_01'
$AvailabilityGroupName = 'AG_HYP_01'
$CollectionID = 'KP10001B'
$SMSProvider = 'kwpcmp001.core.kmtltd.net.au'
#>

$location = Split-Path $MyInvocation.MyCommand.Path -Parent
$LogLocation = "$(Join-Path $(Join-Path $env:windir system32\logfiles) SQLAlwaysOn.log)"
$MailSender = ''
$MailRecipient = ''
$MailPassword = ''
$SMTPServer = ''
...
Function Get-SQLAlwaysOnReplicaNode {
  ...
  foreach($Server in $SQLNodes) {
    if ([string]::IsNullOrEmpty($SQLInstance)) {
      ...
      $connectionString = "Data Source=$Server\$SQLInstance;Initial Catalog=Master;Integrated Security=true;"
    }
...
Function Check-AGHealth {
  ...
  if (! $AlwaysOnInstance) {
    $objHealth = Test-SqlAvailabilityGroup -Path "SQLServer:\SQL\$PrimaryReplicaServer\Default\availabilitygroups\$AGName"
  }
  else {
...
Function Check-ReplicaHealth {
  ...
  if (! $AlwaysOnInstance) {
    Set-Location "SQLServer:\SQL\$PrimaryReplicaServer\Default\availabilitygroups\$AvailabilityGroupName\AvailabilityReplicas"
  }
  else {
    Set-Location "SQLServer:\SQL\$PrimaryReplicaServer\availabilitygroups\$AvailabilityGroupName\AvailabilityReplicas"
  }
  $objReplicaHealth =  Get-ChildItem | Test-SqlAvailabilityReplica
...
Function Check-DBSynchronisation {
  ...
  if (! $AlwaysOnInstance) {
    Set-Location "SQLServer:\SQL\$PrimaryReplicaServer\Default\availabilitygroups\$AvailabilityGroupName\DatabaseReplicaStates"
  }
  else {
    Set-Location "SQLServer:\SQL\$PrimaryReplicaServer\availabilitygroups\$AvailabilityGroupName\DatabaseReplicaStates"
  }
  $DBHealthStates = Get-ChildItem | Test-SqlDatabaseReplicaState
...
<# Not needed at Kmart, if needed in the Future, change Backup locations
Function Backup-AvailabilityDB {
  [CmdletBinding()]
  param()
  [int]$global:errorcount = 0
  Set-Location "SQLServer:\SQL\$PrimaryReplicaServer\availabilitygroups\Default\$AvailabilityGroupName\AvailabilityDatabases"
  $DBs = Get-ChildItem -Path "SQLServer:\SQL\$PrimaryReplicaServer\availabilitygroups\Default\$AvailabilityGroupName\AvailabilityDatabases"
  foreach ($DB in $DBs) {
...
Function Disable-AutomaticFailover {
  ...
  try {
    Set-SqlAvailabilityReplica -AvailabilityMode 'SynchronousCommit' -FailoverMode 'Manual' -Path SQLSERVER:\Sql\$PrimaryReplicaServer\Default\AvailabilityGroups\$AvailabilityGroupName\AvailabilityReplicas\$NewReplicaString
  }
  catch {
    Write-Error $_
  }
...
If ($SendEmail) {
  ...
  $mycreds = New-Object System.Management.Automation.PSCredential ($MailSender, $secpasswd)
  
  Send-MailMessage -Body 'Transcript of SQL AlwaysOn Patching Process' -Subject 'SQL AlwaysON Completion Message' -From $MailSender -SmtpServer $SMTPServer -UseSsl -Credential $mycreds -To $MailRecipient -Attachments $LogLocation
}
